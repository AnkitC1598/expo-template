...
const fileExists = (filename) => fs.existsSync(path.resolve(process.cwd(), filename));
...
if (fileExists('app.json')) {
	...
	const configFile = fileExists('app.config.js') ? 'app.config.js' : 'app.config.ts';
	...
	console.error('❌ Error: No app.json, app.config.js, or app.config.ts found.');
	process.exit(1);
}

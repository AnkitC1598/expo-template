...
const DEFAULT_TIMEOUT_ERROR_MESSAGE =
	"Oops! Network is unstable. Please retry.";
...
class AxiosManager {
	...
	createInstance({
		...
		Object.values(INSTANCE_CONFIGURATIONS).forEach(({ suffix, options }) => {
			this.instances[`${name}${suffix}`] = createInstanceWithInterceptors(
				axiosConfig,
				options,
			);
		});
